...
### v1.0.4
- **앱 버전 동적 표시**: 하드코딩 되어 있던 마이페이지의 앱 버전을 `package_info_plus` 패키지를 적용하여 실제 빌드 버전과 넘버를 연동 및 렌더링하도록 개선했습니다.
- **사진 데이터 영구 보전 처리**: 앱 재빌드 및 업데이트 시 iOS의 앱 샌드박스 UUID 변경이나 임시 폴더 초기화로 인해 사진 데이터가 유실되는 문제를 해결했습니다. 이제 모든 사진은 앱의 영구 문서 폴더(Application Documents Directory)에 안전하게 복사되어 저장됩니다.
- **UI 및 버전 업데이트**: 운동 기록 화면의 안내 메시지 줄바꿈 등을 개선하고 버전을 업데이트했습니다.
